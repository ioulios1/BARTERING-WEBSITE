<div class="dropdown">
    <button class="btn btn-primary dropdown-toggle" id="ddBtn" type="button" data-toggle="dropdown">Κατηγορίες</button>
    <ul class="dropdown-menu nav nav-pills nav-stacked span2" style="color:black;">

    <?php $k=0;mysqli_data_seek($tree_parse,0);?>
    <?php while($cat_row = $tree_parse2->fetch_assoc()) : $row[$k] = $cat_row; ?>
            ...
            <ul class="dropdown-menu">
                ...
                <?php if(strcmp($row[$k-1]['lev3'],$row[$k]['lev3']) && $row[$k]['leaf3']==0) :?>
                    ...
                    <?php if(strcmp($row[$k-1]['lev4'],$row[$k]['lev4'])) :?>
                        <!--salamalekoum -->
                        <li><a tabindex="-1" onclick='<?php echo 'addCategory("'.$row[$k]['lev4'].'",'. $row[$k]['cat4'].')';?>' href="#"><?php echo $row[$k]['lev4']; ?></a></li>
                    <?php endif; ?>

                <?php elseif(strcmp($row[$k-1]['lev3'],$row[$k]['lev3']) && $row[$k]['leaf3']==1) : ?>
                    <li><a tabindex="-1" onclick='<?php echo 'addCategory("'.$row[$k]['lev3'].'",'. $row[$k]['cat3'].')';?>' href="#"><?php echo $row[$k]['lev3']; ?></a></li>
                <?php endif; ?>
            <?php elseif(strcmp($row[$k-1]['lev2'],$row[$k]['lev2']) && $row[$k]['leaf2']==1 )  : ?>
                <li><a tabindex="-1" onclick='<?php echo 'addCategory("'.$row[$k]['lev2'].'",'. $row[$k]['cat2'].')';?>' href="#"><?php echo $row[$k]['lev2']; ?></a></li>
            <?php endif; ?>
...
<div id="category_error_msg" style="font-size: 70%;color:red;"><br></div>

<input type="hidden" id="category" name="category" value="0" required>